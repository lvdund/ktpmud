...
<body>
<nav class="breadcrumb"><a href="<?=base_url()?>/user"><i class="Hui-iconfont">&#xe67f;</a></i> Front page<span class="c-gray en">&gt;</span>Appointment management <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
		...
		<tbody>
			<tr class="text-c">
				<?php foreach (array_reverse($ph) as $item) : ?>
					<tr class="text-c">
                        <?php if ($item['idBenhNhan'] !== session()->get('tbenhnhan')['idBenhNhan']||$item['tinhTrangPH'] =='done') {continue;} ?>
						<td><?= $item['idPH'] ?></td>
                        ...
                        <td>
                        <a href="<?=base_url()?>/user/deleteAppointment/<?=$item['idPH']?>" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        </td>
					</tr>
				<?php endforeach ?>
			</tr>
        ...
        <tbody>
			<tr class="text-c">
				<?php foreach (array_reverse($ph_check) as $item) : ?>
					<tr class="text-c">
                    <?php if ($item['idBenhNhan'] !== session()->get('tbenhnhan')['idBenhNhan']) {continue;} ?>
						<td><?= $item['idPH'] ?></td>
						<td><?= $item['tenBenhNhan'] ?></td>
						<td><?= $item['tenDichVu'] ?></td>
                        <td><?= $item['tenKhoa'] ?></td>
						...
						<td><?= $item['tinhTrangPH'] ?></td>
                        <td>
                        <a href="<?=base_url()?>/user/deleteAppointment/<?=$item['idPH']?>" class="btn btn-danger radius btn-del-comfirmm"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        </td>
					</tr>
				<?php endforeach ?>
			</tr>
